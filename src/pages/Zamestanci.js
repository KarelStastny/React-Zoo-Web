...
const Zamestanci = () => {
  ...
  return (
    <div className="zoo-zamestanci-page">
      <HeddingBodyLayout />
      <div className="zoo-zamestnaci row">
        {employee.map((one, index) => {
          return (
            <div key={index} className="one-person">
              <img
                src={one.picture.large}
                alt={`${one.name.title} ${one.name.first} ${one.name.last}`}
              />
              <article>
                <h2>{`${one.name.title}. ${one.name.first} ${one.name.last}`}</h2>
                <p>
                  Lorem ipsum dolor sit amet consectetur adipisicing elit.
                  Doloremque vo Tempore nam, impedit facere sunt quam maiores
                  ullam rem veritatis unde facilis beatae temporibus corporis.
                </p>
              </article>
            </div>
          );
